...
def generateItemProfiles(R,d,seed,sparkContext,N):
    ...
    """
    # Generating Item Profiles
    I = R.map(lambda (i,j, rij): j).distinct(numPartitions = N)
    numItems = I.count()
    randRDD = RandomRDDs.normalVectorRDD(sparkContext, numItems, d, numPartitions=N, seed=seed)
    I = I.zipWithIndex().map(swap)
    randRDD = randRDD.zipWithIndex().map(swap)
    return I.join(randRDD, numPartitions = N).values()
...
def joinAndPredictAll(R,U,V,N):
    ...
    """
    #u = U.map(lambda(i,ui):ui)
    #v = V.map(lambda(j,vj):vj)
    #r = R.map(lambda(i, j, rij): rij)
    #ru = r.zip(u)
    #ruv = ru.zip(v)
    #predictionDiff = ruv.map(lambda((r,u),v): pred_diff(r,u,v))
    
    #ij = R.map(lambda (i,j,rij):(i,j))
    #ijp = ij.zip(predictionDiff)
    #ijpuv = ijp.zip(u).zip(v) 
    
    #finalRDD = ijpuv.map(lambda((((i,j),p),u),v): (i, j, p, u, v))
    #return finalRDD
    
    I = R.map(lambda(i, j, r): (i,(j, r)))
    J = I.join(U, numPartitions = N).map(lambda (i, ((j,r), u)): (j, (i,r,u)))
    ijruv = J.join(V, numPartitions=N).map(lambda(j, ((i,r,u), v)):(i,j,r,u,v))
    ijpuv = ijruv.map(lambda(i,j,r,u,v): (i,j,pred_diff(r,u,v),u,v))
    return ijpuv  
...
def SE(joinedRDD):
    ...
    """
    SE_results = joinedRDD.map(lambda (i,j,p,u,v): p**2).reduce(add)
    return SE_results
...
def normSqRDD(profileRDD,param):
    ...
    SqRDD = profileRDD.map(lambda (i,u): param * u.dot(u)).reduce(add)
    return SqRDD    
...
def adaptU(joinedRDD,gamma,lam,N):
    ...
    """
    #keyValuePair = joinedRDD.map(lambda(i,j,d,u,v): (i,(j,d,u,v)))
    #adaptU = keyValuePair.map(lambda (i,j,d,u,v): (i, (u-gamma*(2*d*v + 2*lam*u)))).reduceByKey(lambda(x, y): x+y)
    
    #adaptU = joinedRDD.map(lambda (i,j,d,u,v): (i, (u-gamma*(2*d*v + 2*lam*u))))
    #return adaptU
    i = joinedRDD.map(lambda (i,j,delta,u,v):(i,u)).reduceByKey(lambda i,u : i)
    gradient = joinedRDD.map(lambda (i,j,delta,u,v) : (i,gradient_u(delta,u,v)+2*lam*u)).reduceByKey(add)
    return i.join(gradient,numPartitions=N).map(lambda (i,(u,x)) : (i,(u-gamma*x)))
...
def adaptV(joinedRDD,gamma,mu,N):
    ...
    """
    #keyValuePair = joinedRDD.map(lambda(i,j,d,u,v): (i, (j,d,u,v)))
    #adaptV = keyValuePair.map(lambda (i,j,d,u,v): (i, (u-gamma*(2*d*u + 2*mu*v)))).reduceByKey(lambda(x,y):x+y)
    
    #adaptV = joinedRDD.map(lambda (i,j,d,u,v): (i, (u-gamma*(2*d*u + 2*mu*v))))
    #return adaptV
    i = joinedRDD.map(lambda (i,j,delta,u,v):(i,v)).reduceByKey(lambda i,v : i)
    gradient = joinedRDD.map(lambda (i,j,delta,u,v) : (i,gradient_v(delta,u,v)+2*mu*v)).reduceByKey(add)
    return i.join(gradient,numPartitions=N).map(lambda (i,(v,x)) : (i,(v-gamma*x)))
